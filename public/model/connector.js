class Connector{
  ...
  constructor(you){
    ...
    this.dir = [{x:0,y:0,r:0},{x:1,y:0,r:0},{x:1,y:1,r:45,off:{x:0.166,y:-0.085}},{x:0,y:1,r:90,off:{x:0.333,y:0}},{x:-1,y:1,r:135,off:{x:0.4,y:0.166}},{x:-1,y:0,r:180},{x:-1,y:-1,r:225,off:{x:0.166,y:0.4}},{x:0,y:-1,r:270,off:{x:0,y:0.333}},{x:1,y:-1,r:315,off:{x:-0.085,y:0.166}}]
  }

  move(from,to,tS){
    const adjust = this.dir.find((p) => to.x-from.x == p.x && to.y-from.y == p.y)
    this.el.setAttribute('transform',`translate(${to.x * tS + tS/3 + (adjust.off ? adjust.off.x * tS : 0)},${to.y * tS + tS/3 + (adjust.off ? adjust.off.y * tS : 0)}) scale(${tS/200}) rotate(${adjust.r})`);
  }
